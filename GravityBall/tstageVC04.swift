...
class tstageVC04: UIViewController,AVAudioPlayerDelegate{
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(sv)
        let ss = tstage04(size: sv.frame.size)
        ...
        backButton.addTarget(self, action: #selector(tstageVC04.back(_:)), for: .touchUpInside)
        view.addSubview(backButton)
        
        //説明表示
        disc = 0
        let Description = UIButton(frame: CGRect(x: xx, y: yy, width: xx * 8, height: yy * 16))
        Description.setImage(UIImage.init(named: "t4_1"), for: .normal)
    ...
    @objc func dis(_ sender: UIButton) {
        disc = 1 + disc
        if disc == 1{
            sender.setImage(UIImage.init(named: "t4_2"), for: .normal)
        }
        if disc == 2{
            sender.setImage(UIImage.init(named: "t4_3"), for: .normal)
        }
        if disc == 3{
            sender.setImage(UIImage.init(named: "t4_4"), for: .normal)
        }
        if disc == 4{
