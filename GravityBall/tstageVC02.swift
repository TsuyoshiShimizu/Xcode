...
class tstageVC02: UIViewController,AVAudioPlayerDelegate{
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(sv)
        let ss = tstage02(size: sv.frame.size)
        ...
        backButton.addTarget(self, action: #selector(tstageVC02.back(_:)), for: .touchUpInside)
        view.addSubview(backButton)
        
        //説明表示
        disc = 0
        let Description = UIButton(frame: CGRect(x: xx, y: yy, width: xx * 8, height: yy * 16))
        Description.setImage(UIImage.init(named: "t2"), for: .normal)
